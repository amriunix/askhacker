...
command | description
:-| :-
`curl -I https://askhacker.com` | `curl` sends a request
`curl -v -I https://askhacker.com` | `curl` request with details
`curl -X GET https://askhacker.com` | use explicit http method for `curl`
`curl --noproxy 127.0.0.1 http://www.stackoverflow.com` | `curl` without http proxy
`curl --connect-timeout 10 -I -k https://askhacker.com` | `curl` has no timeout by default
`curl --verbose --header "Host: www.mytest.com:8182" askhacker.com` | `curl` get extra header
